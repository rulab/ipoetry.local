 ...
 *
 * @author d.krasavin
 * контроллер для формы авторизации нового пользовтеля
 */
class SigninController extends LoginController{
	...
	//if (!isset($this->config))
        ...
        //if ($request->get('user') && $request->get('password') && !($request->get('data_modification'))) {
                ...
                $UserLogin=new UserSignin();
                $UserSigninType=new UserSigninType($this->get('router'),$this->session,$request);
                $form =$this->createForm($UserSigninType, $UserLogin);
        ...
        if (isset($form)){
            $html = $this->render('IpoetryBundle:Signin:signin_form.html.twig',array('form' => $form->createView()));//,'captcha'=>$CaptchaView
        }
        //echo $this->captchabuilder->getPhrase().' ';
        //if($form->isValid()) {
            return $html;            
